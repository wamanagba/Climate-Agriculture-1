...
download.file(paste("http://iridl.ldeo.columbia.edu/SOURCES/.IRI/.Analyses/.SPI/.SPI-CAMSOPI_3-Month/X/-25/0.5/55/GRID/Y/-40/0.5/40/GRID/T/%28",First_Month,"-",Month$Month_Abbr[1],"%20",Year,"%29%28",First_Month,"-",Month$Month_Abbr[1],"%20",Year,"%29RANGEEDGES/data.nc",sep=""),mode="wb",paste("Data/SPI_",First_Month,"_",Month$Month_Abbr[1],".nc",sep=""))

Data<-nc_open(filename = paste("Data/SPI_",First_Month,"_",Month$Month_Abbr[1],".nc",sep=""))
...
Data_interpolted_SPI<-raster::disaggregate(SPI_Raster_format,5,method="bilinear")
...
rio::export(Data_df_SPI,paste("Results/SPI_",First_Month,"-",Month,".csv",sep=""))

####################Climatology of the season###################################


download.file(paste("http://iridl.ldeo.columbia.edu/SOURCES/.NOAA/.NCEP/.CPC/.CAMS_OPI/.v0208/.mean/.prcp/T/(",First_Month,"%201981)/(",Month$Month_Abbr[1],"%202010)/RANGEEDGES/Y/-40/0.5/40/GRID/X/-25/0.5/55/GRID/T/",3,"/runningAverage/T/12/STEP/%5BT%5Daverage/",89,"/mul/data.nc",sep=""),mode="wb",paste("Data/Climatology_",First_Month,"_",Month,".nc",sep=""))

Climatology_Raster_format<-raster::raster(x =paste("Data/Climatology_",First_Month,"_",Month$Month_Abbr[1],".nc",sep=""))

Data_interpolted_clim<-raster::disaggregate(Climatology_Raster_format,5,method="bilinear")
...
rio::export(Data_df_SPI[,c("x","y","SPI","Climatology")],paste("Results/SPI_",First_Month,"-",Month,".csv",sep=""))
...
jpeg(filename = paste("Products/Graphs/",Month_name,"/SPI_",First_Month,"_",Month$Month_Abbr[1],"_",Year,".jpeg",sep=""),
     width = 11,
     height = 12,
     units = "in",
     res=300)
